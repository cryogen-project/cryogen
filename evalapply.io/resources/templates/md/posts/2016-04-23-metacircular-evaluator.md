...
<img src="http://i.imgur.com/qxpVOq1.png" 
title="AST for (+ 1 2 (* 3 4))" 
alt="AST for (+ 1 2 (* 3 4))"/> 
...
A Lisp is a language that is written in an AST, when the language is interpreted or compiles these code trees can be manipulated.
...
It is interesting to note that these other data structures are implemented in Clojure in such a way that there are immutable (unchangeable). 
...
This is a recursive definition (like `map` above) where we evaluate construct a new list with as a first argument the first operand evaluated in the environment, and as a tail the rest of the operands evaluated in the environment. 
...
The second element itself are the parameters `(a b c d)`. 
...
Apply is a procedure that takes as input a procedure and its arguments, and produces a produces an expression and an environment:
...
If you would like to see a full evaluation of an expression by hand, I can recommend watching the second part of [Lecture 7A of SICP](https://www.youtube.com/watch?v=0m6hoOelZH8&t=36m40s). 
...
As a footnote to the meta-circular evaluator in SICP](https://mitpress.mit.edu/sicp/full-text/book/book-Z-H-26.html#footnote_Temp_510) states:
#[T]here will remain important aspects of the evaluation process that are not elucidated by our evaluator. The most important of these are the detailed mechanisms by which procedures call other procedures and return values to their callers. We will address these issues in chapter 5, where we take a closer look at the evaluation process by implementing the evaluator as a simple register machine. 
WHAT IS THIS MAGIC THIS SUSSMAN CHARACTER IS TELLING ME? What a strange nonsense here.

After all, he perported to explain to me Lisp, and he wrote me a Lisp program.
The lisp program was intended to a an interpreter for Lisp, but you need a Lisp interpreter in order to understand that program. How could that program told me anything there is to be known about Lisp. How is that not completely vacuous?

It is a very strange thing. It does not tell me anything at all.
This whole thing is sort of like these Escher's hands that we see here:
EVAL/APPLY draw each other and construct the real thing which draws itself.

I am going to try to convince you that this means something and why you don't need definitions. There are inessential in a mathemathical sense for doing all the things we are doing in computing. 

Denotational semantics.
The Y combinator.
Define is a side-effect. 
define modifies an environment structure. Changes the frame that the define is executed in. There ar emay reasons for that. Making an interactive system.

Extending the languages:
[Homoiconic](https://en.wikipedia.org/wiki/Homoiconicity) AST
Homoiconic languages typically include full support of syntactic macros allowing the programmer to express program transformations in a concise way. 

[First-class macro's](http://matt.might.net/articles/metacircular-evaluation-and-first-class-run-time-macros/)

I DONT UNDERSTAND MACROS.

Fixed point.
I don't understand this. Paul Graham named his company after it. Y Combinator.
...
SICP in turn is based on the [Lambda papers](http://library.readscheme.org/page1.html) by Guy Steele and Gerald Jay Sussman.
